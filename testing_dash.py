...
def generate_table(deployed_data):
    print deployed_data
    for i in range(len(deployed_data.keys())-1):
        print i
    return html.Table(className='reaponsive-table',
        ...
        children=[
            html.Thead(
                html.Tr([
                    html.Th('Module',style={'colspan':'4',}),
                    html.Th('Buildnumber')])
                ), 
        ...
        # Body
            html.Tbody(
                [html.Tr([
                    html.Td(key,style={'colspan':'4','padding-top':'4px', 'padding-bottom':'4px', 'text-align':'centre'})
                    
                    ]) for key in deployed_data.keys() if key!='Cartridges']
                )
    ])
